...
#[command(
    ...
    after_help = "Examples:\n\n      # Add streamable HTTP server:\n      dvadva-agent mcp add --transport http context7 https://mcp.context7.com/mcp --header \"CONTEXT7_API_KEY: ctx7sk-your-key\"\n\n      # Add streamable HTTP server with OAuth authorization:\n      dvadva-agent mcp add --transport http --auth oauth linear https://mcp.linear.app/mcp\n\n      # Add stdio server:\n      dvadva-agent mcp add --transport stdio chrome-devtools -- npx chrome-devtools-mcp@latest"
)]
...
fn describe_mcp_server(name: &str, server: &Value, auth_required: bool) -> String {
    ...
    if let Some(url) = server.get("url").and_then(Value::as_str) {
        ...
        if auth_required {
            line.push_str(" [authorization required - run: dvadva-agent mcp auth ");
            line.push_str(name);
            line.push(']');
        }
        return line;
