...
async def _login_kimi_code(config: Config) -> bool:
...
def current_model_key(config: Config) -> str | None:
...
async def login(app: Shell, args: str) -> None:
    ...
    if platform.id == KIMI_CODE_PLATFORM_ID:
        ok = await _login_kimi_code(config)
    else:
...
async def logout(app: Shell, args: str) -> None:
    ...
    if not config.is_from_default_location:
        ...
        return
    model_key = current_model_key(config)
    if not model_key:
