...
Drive a `dvadva-agent` running on a **remote machine** from the local
`dvadva-tui` / `inkvizitor`, with **zero changes to `dvadva-agent`'s wire surface**
...
JSON from stdin and writes to stdout (`server/dvadva-agent/src/wire/server.rs`);
...
Interim proof it works today: `dvadva-tui --agent-bin ssh user@host dvadva-agent -w /remote/dir`
...
```
local:  dvadva-tui/gui ─stdio─▶ dvadva-bridge local ──TCP/SSH tunnel──▶ dvadva-bridge remote ─stdio─▶ dvadva-agent
                              (loopback listen)    (network leg)    (spawns dvadva-agent)   (unchanged)
```

- **Remote daemon**: listens on a TCP port; per connection spawns `dvadva-agent`
...
- One crate: `remote/dvadva-bridge`; one binary with `local` / `remote`
...
1. **Scaffold** `remote/dvadva-bridge` as a workspace member (add to root
   `Cargo.toml` members). Deps: tokio (+ clap/serde for args) only.
2. **Relay core**: accept loop + spawn `dvadva-agent` with piped stdio +
...
4. **Frontend wiring**: `dvadva-tui` + `inkvizitor` accept a remote endpoint
...
5. **Remote session listing**: the resume menu reads `~/.kimi` in-process
   ...
   answers a listing query using `dvadva_agent::session::Session::list` (a
...
Landed as `remote/dvadva-bridge` (one crate, one binary, two subcommands) plus
a stream transport in `wire-client`:

- **Bridge framing** (`remote/dvadva-bridge/src/proto.rs`, mirrored client-side
...
- **The work-dir default**: a frontend on another OS cannot name a path that
  ...
  This is why inkvizitor's `+` opens no folder picker over a remote bridge.
...
- **Frontends**: `--remote <host:port>` / `$KIMI_REMOTE` (stripped from
  agent args by `AgentLaunch`); `dvadva-tui`/`inkvizitor` branch to
  `connect_tcp`, and the resume menu lists through the daemon
  (`spawn_remote_session_listing`) instead of local `~/.kimi`.
- **Session listing** on the daemon reuses `dvadva_agent::session::Session`
...
- **`~/.kimi/bridge.toml`**, one file with two disjoint halves: `[serve]`
  (read by `dvadva_bridge::config`, so a systemd unit is a bare
  `dvadva-bridge remote`) and `[[remotes]]` (read by `wire_client::remotes`:
...
- **`wire_client::tunnel`** runs the ssh process as a managed child (spawn,
  ...
  stderr tail.
- **Sessions became per-tab** in inkvizitor (`SessionTarget`): `--remote` now
...
- Loopback e2e (`remote/dvadva-bridge/tests/`): frame handling, arg passing,
...
- Manual smoke (done, Windows): `dvadva-bridge remote` + the real `dvadva-agent`
...
./dvadva-bridge remote --listen 127.0.0.1:9000   # agent resolved: --agent-bin /
...
./dvadva-tui --remote 127.0.0.1:9000             # no -w: the daemon defaults
./inkvizitor  --remote 127.0.0.1:9000            # paths resolve on the VPS
```

The `local` subcommand (`dvadva-bridge local --upstream 127.0.0.1:9000`) is
