//! `kimi-bridge` — the relay daemon pair that carries a `kimi-agent` wire
...
//!   connection it spawns `kimi-agent` with caller-chosen arguments and
...
//! the kimi-agent wire protocol — opaque newline-delimited JSON that flows
